...
public class TomAvatar extends SuperAvatar {
    ...
    public Direction startSpiral(ArrayList<SpaceInfo> spacesInRange){
        ...
        while(myCoordinate.getX() > 1){
            if(directionCounter == 0 & stepCounter <= 3 + storePerceptionRange){
                if(stepCounter == 3 + storePerceptionRange){
                    stepCounter = 0;
                    directionCounter = 1;
                }        
                return Direction.RIGHT;
            } else if(directionCounter == 1 & stepCounter <= 3 + storePerceptionRange){
                if(stepCounter == 3 + storePerceptionRange){
                    stepCounter = 0;
                    directionCounter = 2;
                }
                return Direction.UP;
            } else if(directionCounter == 2 & stepCounter <= (3 + storePerceptionRange) * 2){
                if(stepCounter == (3 + storePerceptionRange) * 2 ){
                    stepCounter = 0;
                    directionCounter = 3;
                }
                return Direction.LEFT;
